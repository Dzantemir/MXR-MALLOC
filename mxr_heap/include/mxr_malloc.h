...
#define MXR_IRAM_CAPS_DEFAULT \
     ...
     * Descriptor helpers.
     */
    static inline uint16_t MXR_INLINE_ATTR mxr_desc_off(const mxr_desc_t *d)
    {
        return (uint16_t)(d->off_flags & MXR_OFF_MASK);
    }

    static inline uint16_t MXR_INLINE_ATTR mxr_desc_len(const mxr_desc_t *d)
    {
        return (uint16_t)((d->len_flags & MXR_LEN_MASK) + 1u);
    }

    static inline uint16_t MXR_INLINE_ATTR mxr_desc_off_flags(const mxr_desc_t *d)
    {
        return (uint16_t)(d->off_flags & MXR_OFF_FLAGS_MASK);
    }

    static inline uint16_t MXR_INLINE_ATTR mxr_desc_len_flags(const mxr_desc_t *d)
    {
        return (uint16_t)(d->len_flags & MXR_LEN_FLAGS_MASK);
    }

    static inline bool MXR_INLINE_ATTR mxr_desc_is_iram(const mxr_desc_t *d)
    {
        return (d->off_flags & MXR_OFF_FLAG_IRAM) != 0;
    }

    static inline bool MXR_INLINE_ATTR mxr_desc_is_exec(const mxr_desc_t *d)
    {
        return (d->len_flags & MXR_LEN_FLAG_EXEC) != 0;
    }

    static inline uint16_t MXR_INLINE_ATTR mxr_desc_make_key(uint16_t off_units, bool iram)
    {
        if (iram)
        {
    ...
    static inline void MXR_INLINE_ATTR mxr_desc_clear(mxr_desc_t *d)
    {
        d->off_flags = 0;
        d->len_flags = 0;
    }

    static inline void MXR_INLINE_ATTR mxr_desc_set(
        mxr_desc_t *d,
        uint16_t off_units,
        uint16_t len_units,
        uint16_t off_flags,
