 ...
 *
 *  Split descriptor architecture:
 *    s_dram_desc[] — DRAM allocations only
 *    s_iram_desc[] — IRAM allocations only (EXEC + fallback)
 *
 *  Descriptor format (8 bytes):
 *    off_flags  (uint32_t): bits 30..0 = offset bytes
 *    len_flags  (uint32_t): bit 31 = EXEC, bits 30..0 = length bytes
 *
...
#define CONFIG_MXR_IRAM_MAX_DESC 32
#endif

#define MXR_REGIONS_MAX  32
#define MXR_REGIONS_MIN  1

/* Region count from CMake-parsed CONFIG_MXR_REGION_CONFIG */
#ifndef MXR_PARSED_REGION_COUNT
#define MXR_PARSED_REGION_COUNT 3
#endif
 ...
 *
 *  Applied ONLY to the descriptor arrays (s_dram_desc, s_iram_desc).
 *  These are 8-byte structs, so all accesses are 32-bit word
 *  aligned and safe in IRAM.
 *
 *  NOT applied to scalar state variables (bool, uint8_t, uint16_t)
 *  because IRAM does not support byte/half-word access
 *  (LoadStoreAlignment exception on LX106).
 * ================================================================ */
...
typedef struct
{
    uint32_t off_flags;   /* bits30..0 = offset bytes */
